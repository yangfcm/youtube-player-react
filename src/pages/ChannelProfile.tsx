...
export default function ChannelProfile() {
  ...
  return (
    <Box sx={{ px: 2, pt: 1, pb: 4 }}>
      <ProfileTitle>
        <AccessTimeIcon />
        &nbsp;Joined on {formatDate(channelProfile.snippet.publishedAt)}
      </ProfileTitle>
      <ProfileTitle>
        <VideoLibraryIcon />
        &nbsp;{formatNumber(
      ...
      <ProfileTitle>
        <SubscriptionsIcon />
        &nbsp;
        {formatNumber(parseInt(channelProfile.statistics.subscriberCount))}{" "}
        Subscribiers
      </ProfileTitle>
      <ProfileTitle>
        <VisibilityIcon />
        &nbsp;{formatNumber(parseInt(channelProfile.statistics.viewCount))}{" "}
        Views
      </ProfileTitle>
      <Divider sx={{ my: 1 }} />
      <ProfileContent>{channelProfile.snippet.description}</ProfileContent>
    </Box>
  );
}
